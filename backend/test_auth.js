 ...
 *
 * Tests:
 *   1. Student Signup (POST /api/auth/register)
 *   2. Student Login  (POST /api/auth/login)
 *   3. Admin Login    (POST /api/auth/admin/login)
 *   4. GET /api/auth/me (student + admin)
 *   5. Protected routes blocked without token
 *   6. Admin-only routes blocked for students
 *   7. Public routes still work without any token
 *   8. Student can place order as guest (no token)
 *   9. Student can place order with token
...
async function runAuthTests() {
    ...
    try {
        ...
        const badLogin = await req("POST", "/auth/login", {
            email: "ananya@college.edu", password: "wrongpass",
        });
        if (badLogin.status !== 401) fail("Wrong password should return 401");
        ...
        const badAdmin = await req("POST", "/auth/admin/login", {
            username: "canteen_admin", password: "wrongpass",
        });
        if (badAdmin.status !== 401) fail("Wrong admin password should return 401");
        ok(`Wrong admin password correctly rejected ✓`);
        ...
        // No token → 401
        const meNone = await req("GET", "/auth/me");
        if (meNone.status !== 401) fail("No token should return 401");
        ...
        const studentMenu = await req("POST", "/menu", {
            name: "Hack Item", price: 50, category: "Snacks",
        }, studentToken);
        if (studentMenu.status !== 403) fail("Student should get 403 trying to add menu item");
        ok(`Student token on POST /menu → 403 Forbidden ✓`);

        // ────────────────────────────────────────────────────────────
        section("8. GUEST ORDER — No token (public order placement)");
        ...
        // Admin updates status: Waiting → Preparing
        ...
        const newItem = await req("POST", "/menu", {
            name: "Test Chai Special",
            price: 25,
            category: "Beverages",
            stockQuantity: 30,
        }, adminToken);
        if (newItem.status !== 201) fail(`Admin add item failed: ${JSON.stringify(newItem.data)}`);
        ok(`Admin added menu item: "${newItem.data.data.name}" ✓`);
